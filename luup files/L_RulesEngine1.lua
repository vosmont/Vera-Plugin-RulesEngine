...
local VARIABLE = {
	...
	MQTT_CONNECTED = { "urn:upnp-sensor-mqtt-se:serviceId:SensorMqtt1", "mqttServerConnected", true },
	-- Specific RulesEngine
	PLUGIN_VERSION = { "urn:upnp-org:serviceId:RulesEngine1", "PluginVersion", true },
	--PLUGIN_OPTIONS = { "urn:upnp-org:serviceId:RulesEngine1", "PluginOptions", true },
	DEBUG_MODE = { "urn:upnp-org:serviceId:RulesEngine1", "Debug", true },
	LAST_UPDATE = { "urn:upnp-org:serviceId:RulesEngine1", "LastUpdate", true },
	LAST_MAIN_UPDATE = { "urn:upnp-org:serviceId:RulesEngine1", "LastMainUpdate", true },
	...
	NO_LOAD_IF_DISABLED = { "urn:upnp-org:serviceId:RulesEngine1", "NoLoadIfDisabled", true },
}
local indexVariable = {}
...
local DEVICE = {
	MQTT = {
		type = "urn:schemas-sensor-mqtt-se:device:SensorMqtt:1", file = "D_SensorMqtt1.xmll"
	}
}
...
local function getFormatedMethodName( methodName )
	return string.rpad( "(" .. _NAME .. "::" .. tostring(methodName or "log") .. ")", 48 )
end
...
UI = {
	...
	setToolboxConfig = function( toolboxConfig )
		if ( type( toolboxConfig ) ~= "table" ) then
			debug( "Toolbox config is not a table", "UI.addToolboxConfig" )
			return
		end
		local newToolboxConfig = {}
		for _, itemConfig in ipairs( toolboxConfig ) do
			table.insert( newToolboxConfig, itemConfig )
		end
		local jsonToolboxConfig = json.encode( newToolboxConfig )
		debug( "New toolbox config: " .. tostring( jsonToolboxConfig ), "UI.addToolboxConfig" )
		Variable.set( g_deviceId, VARIABLE.TOOLBOX_CONFIG, jsonToolboxConfig )
	end
}
...
Event = {
	...
	onDeviceVariableIsUpdated = function( lul_device, lul_service, lul_variable, lul_value_old, lul_value_new )
		...
		ScheduledTasks.add( { conditionIds = conditionIds }, Conditions.updateStatus, 0, { linkedConditions, { context = context } } )
	end,
	...
	onTimerIsTriggered = function( data )
		...
		if isEnabled() then
			...
			-- Update the status of the conditions (asynchronously to release the lock the fatest possible)
			ScheduledTasks.add( { conditionIds = conditionIds }, Conditions.updateStatus, 0, { linkedConditions, { context = context, event = data } } )
		else
...
ScheduledTasks = {
	...
	getTaskInfo = function (task)
		...
		local taskInfo = {
			timeout = os.date( "%Y-%m-%d %X", task.timeout ),
		...
		for i = #_nextWakeUps, 1, -1 do
			if (_nextWakeUps[i] <= now) then
				if _isLogLevel(4) then
					log("Wake-up #" .. tostring(i) .. "/" .. tostring(#_nextWakeUps) .. " at " .. os.date("%Y-%m-%d %X", _nextWakeUps[i]) .. " (" .. tostring(_nextWakeUps[i]) .. ") is expired", "ScheduledTasks.purgeExpiredWakeUp", 4)
				end
		...
		if ( #_scheduledTasks > 0 ) then
			...
			while ( scheduledTask ) do
				if ( scheduledTask.timeout <= current ) then
					...
					if _isLogLevel(4) then
						log( "Timeout reached for task (still " .. tostring( #_scheduledTasks ) .. " task(s)) : " .. ScheduledTasks.getTaskInfo( scheduledTask ), "ScheduledTasks.execute", 4 )
					end
...
Sun = {
	update = function()
		...
		log( "Sunrise: " .. os.date( "%Y-%m-%d %X", _sunrises[1] ) .. " / " .. ( _sunrises[2] and os.date( "%Y-%m-%d %X", _sunrises[2] ) or "" ), "Sun.update", 2 )
		log( "Sunset : " .. os.date( "%Y-%m-%d %X", _sunsets[1] ) .. " / " ..  ( _sunsets[2] and os.date( "%Y-%m-%d %X", _sunsets[2] ) or "" ), "Sun.update", 2 )
...
do
	...
	ConditionTypes["unknown"] = {
		...
		computeStatus = function (condition)
			log(
				"SETTING WARNING - " .. _getItemSummary(condition) ..
				" - Condition type '" .. tostring(condition.type) .. "' is unknown",
				"ConditionUnknown.computeStatus"
			)
		end
	}
...
log( "DEBUG service:" .. tostring(condition.service) .. ", variable:" .. tostring(condition.variable) .. ", value:" .. tostring(condition.value) .. ", deviceId:" .. tostring(condition.deviceId) .. ", deviceIds:" .. tostring(json.encode(condition.deviceIds)).. ", modifiers:" .. tostring(json.encode(condition.modifiers)), "ConditionValue.init", 5 )
	...
	log( "DEBUG template " .. json.encode( newConditionTemplate ), "ConditionValue.init", 5 )
		...
		computeStatus = function( condition, params )
			local msg = _getItemSummary( condition )
			...
			if ( condition.value ~= nil ) then
				...
				log( "condition.operator:" .. tostring( condition.operator ) .. ", conditionValue:" .. tostring( conditionValue ) .. ", contextValue:" .. tostring( contextValue ), "ConditionValue.computeStatus", 3 )
				if ( ( context.value == nil )
					or ( OPERATOR[condition.operator] == nil )
					...
					msg = msg .. " - Does not respect the condition '{value:" .. tostring(context.value) .. "} " .. tostring(OPERATOR[condition.operator]) .. " " .. tostring(condition.value) .. "'"
					status = 0
				else
					msg = msg .. " - Respects the condition '{value:" .. tostring(context.value) .. "} " .. OPERATOR[condition.operator] .. " " .. tostring(condition.value) .. "'"
				end
			else
			...
			log( msg, "ConditionValue.computeStatus", 3 )
	...
	ConditionTypes["condition_rule"] = {
		...
		computeStatus = function( condition, params )
			...
			log( msg, "ConditionRule.computeStatus", 3 )
	...
	ConditionTypes["condition_time"] = {
		...
		computeStatus = function( condition, params )
			...
			log( msg, "ConditionTime.computeStatus", 3 )

			-- TODO time of untrip (like motion sensor) - for the moment 1 second (should be a parameter ?)
			if ( status == 1 ) then
				ScheduledTasks.add(
					...
					Condition.updateStatus, 1, { condition }
				)
			end
	...
	ConditionTypes["condition_time_between"] = {
		...
		computeStatus = function( condition, params )
			...
			log( msg, "ConditionTimeBetween.computeStatus", 3 )
			return status
		end
	}
	...
	ConditionTypes["condition_inverter"] = {
		...
		computeStatus = function( condition, params )
			...
			local level = 0
			log( _getItemSummary( condition ) .. " - Invert condition " .. _getItemSummary( condition.condition ) .. " (status:" .. tostring( status ) .. ")", "ConditionInverter.computeStatus", 3 )
			if ( status == 1 ) then
				status = 0
			else
				status = 1
...
log( "DEBUG " .. _getItemSummary( condition ) .. " - Actions: "..tostring(json.encode(condition.actions)), "ConditionGroup.init", 5 )
		...
		computeStatus = function( condition, params )
			...
			log( _getItemSummary(condition) .. " - " .. msg, "ConditionGroup.computeStatus", 4 )
	...
	ConditionTypes["condition_sequence"] = {
		...
		computeStatus = function( sequence, params )
			if ( sequence.items == nil ) then
				return
			end
			local params = params or {}
			...
			log( _getItemSummary( sequence ) .. " - " .. msg, "ConditionSequence.computeStatus", 4 )
			return status, level
		end
	}
	...
	-- Condition of type 'function'
	ConditionTypes["condition_function"] = {
		init = function( condition, ruleContext )
			condition.mainType = "Function"
			if (condition.functionContent == nil) then
				return
			end
			local chunk, strError = loadstring( "return function(context) \n" .. condition.functionContent .. "\nend" )
			if ( chunk == nil ) then
				Rule.addError( condition._ruleId, "Init rule condition", "Error in functionContent: " .. tostring( strError ) )
			else
				-- Put the chunk in the plugin environment
				setfenv( chunk, getfenv( 1 ) )
				condition.callback = chunk()
				condition.functionContent = nil
			end
		end,

		check = function( condition )
			if not _checkParameters( condition, { "callback" } ) then
				return false
			end
			return true
		end,
	...
	-- Condition of type 'last_update'
	ConditionTypes["condition_last_update"] = {
		init = function( condition, ruleContext )
			condition.mainType = "Function"
			condition.interval = _getIntervalInSeconds( condition.interval, condition.unit )
		end,
		...
		computeStatus = function( condition, params )
			local parent = condition._parent
			local context = parent._context
			local elapsedTime = os.difftime( os.time(), ( context.lastStatusUpdateTime or context.lastUpdateTime or 0 ) )
			local msg = _getItemSummary( condition ) .. " - " .. _getItemSummary( parent )
			local status = 1
			if (
				( OPERATOR[condition.operator] == nil )
				or ( ( ( condition.operator == "LT" ) or ( condition.operator == "LTE" ) ) and ( contextValue > conditionValue ) )
				or ( ( condition.operator == "LTE" ) and ( elapsedTime > condition.interval ) )
				or ( ( condition.operator == "GTE" ) and ( elapsedTime < condition.interval ) )
			) then
				-- Interval is not respected
				msg = msg .. " - Does not respect the condition '{last update:" .. tostring(elapsedTime) .. "s} " .. tostring(OPERATOR[condition.operator]) .. " " .. tostring(condition.interval) .. "'"
				status = 0
			else
				msg = msg .. " - Respects the condition '{last update:" .. tostring(elapsedTime) .. "s} " .. OPERATOR[condition.operator] .. " " .. tostring(condition.interval) .. "'"
			end

			local level = 0
			if ( status == 1 ) then
				if ( condition.modifiers and condition.modifiers.level ) then
					level = condition.modifiers.level or 0
				end
			end

			log( msg, "ConditionLastUpdate.computeStatus", 3 )
			return status, level
		end
	}
...
--  Condition of a rule
--  Condition of a group of actions
-- **************************************************
...
Condition = {
	init = function( condition, ruleId, parent, ruleContext )
		...
		-- Add to index
		Conditions.addToIndex( condition )
		-- Modifiers
	...
	updateStatus = function( conditionId, params )
		local condition = Conditions.get( conditionId )
		if ( condition == nil ) then
			return false
		end
		...
		-- Update the status
		local hasStatusChanged = Condition.setStatus( condition, status, params )

		-- Execute the condition actions of the parent if needed
		-- TODO : enlever condition hasParentStatusChanged mais ne pas exécuter action parent au changement de statut (déjà fait par enfants)
		...
		-- Update the parent
		local hasParentStatusChanged = false
		-- TODO : if lastUpdateTime > lastStatusUpdateTime transmettre params.context pour update
		local parent = condition._parent
		if ( ( parent ~= nil ) and not params.noPropagation and ( hasStatusChanged or params.levelHasChanged or not parent.hasOnlyTriggers ) ) then
			log( msg .. " - Update parent " .. _getItemSummary( parent ), "Condition.setStatus", 3 )
			local updateMethod, updateParams
			if ( parent.mainType == "Rule" ) then
				updateMethod = Rule.updateStatus
			elseif ( parent.mainType ~= "ActionGroup" ) then
				updateMethod = Condition.updateStatus
				updateParams = { lastUpdateTime = condition._context.lastStatusUpdateTime, context = params.context }
			else
				log( msg .. " - No update for parent " .. parent.mainType, "Condition.setStatus", 3 )
			end
			if updateMethod then
				if isConditionActionToDo then
					ScheduledTasks.add(
						{ ruleId = condition._ruleId, summary = "Update status of " .. _getItemSummary( parent ) },
						updateMethod, 0, { parent, updateParams }
					)
				else
					hasParentStatusChanged = updateMethod( parent, updateParams )
				end
			end
		end

		local hasSomethingChange = ( hasStatusChanged or hasToCheckConditionStatusLater or params.isForced )
		if ( hasSomethingChange and params and params.saveRulesInfosOnChange ) then
			RulesInfos.save( { condition._ruleId } )
		end

log( _getItemSummary( condition ) .. " " .. json.encode( params ), "Condition.updateStatus", 5 )
		return hasSomethingChange
	end,

	-- Modify status of the condition
	setStatus = function( conditionId, status, params )
		local condition = Conditions.get( conditionId )
		if ( condition == nil ) then
			return false
		end
		...
		local msg = _getItemSummary( condition )
		local hasStatusChanged = false
		local params = params or {}
--log( _getItemSummary( condition ) .. " " .. json.encode( params ), "Condition.setStatus", 4 )
...
local _indexConditionsById = {}

Conditions = {
	get = function( conditionId )
		local condition
		if ( type( conditionId ) == "string" ) then
			-- Try to get by id
			condition = _indexConditionsById[ conditionId ]
			if ( condition == nil ) then
				log( "WARNING - Condition #" .. conditionId .. " is unknown", "Conditions.get" )
			end
		elseif ( type( conditionId ) == "table" ) then
			if ( conditionId.id ~= nil ) then
				condition = Conditions.get( conditionId.id )
				if ( condition ~= conditionId ) then
					error( "Given condition is not the condition added with this id", "Conditions.get" )
				end
			else
				error( "Given condition has not been retrieved", "Conditions.get" )
			end

		else
			error( "Parameter #1 is not a table", "Conditions.get" )
		end
		return condition
	end,

	addToIndex = function( condition )
		_indexConditionsById[condition.id] = condition
	end,

	removeRule = function( ruleId )
		local nbRemoved = 0
		for conditionId, condition in pairs( _indexConditionsById ) do
			if ( condition._ruleId == ruleId ) then
				nbRemoved = nbRemoved + 1
				_indexConditionsById[conditionId] = nil
			end
		end
		log( "Unregister conditions for rule #" .. tostring( ruleId ) .. ": " .. tostring( nbRemoved ) .. " condition(s) removed from index", "Conditions.removeRule", 2 )
	end,
...
do
	...
	_actionTypes["action_mqtt"] = {
		getMqttDeviceId = function()
			-- Search for the MQTT plugin
			if ( _actionTypes["action_mqtt"].mqttDeviceId == nil ) then
				for deviceId, device in pairs( luup.devices ) do
					if ( device.device_type == DEVICE.MQTT.type ) then
						log( "Found MQTT plugin device #" .. tostring( deviceId ), "ActionType.action_mqtt.init", 4 )
						_actionTypes["action_mqtt"].mqttDeviceId = deviceId
						return deviceId
					end
				end
			else
				return _actionTypes["action_mqtt"].mqttDeviceId
			end
		end,
		init = function( action )
			action.mqttDeviceId = _actionTypes["action_mqtt"].getMqttDeviceId()
			log( "Action " .. tostring( json.encode( action ) ), "ActionType.action_mqtt.init", 4 )
		end,
		check = function( action )
			if not _checkParameters( action, { "mqttDeviceId", "topic", "payload" } ) then
				return false
			end
			return true
		end,
		execute = function( action, context )
			-- Check if the plugin MQTT is connected
			if ( Variable.get( action.mqttDeviceId, VARIABLE.MQTT_CONNECTED ) ~= "1" ) then
				Rule.addError( action._ruleId, "Execute rule actions", "MQTT plugin is not connected" )
				return false
			end
			-- Call luup action
			local payload = RulesEngine.Tools.format( action.payload, context )
			History.add( action._ruleId, "RuleAction", _getItemSummary( action ) .. " - Send MQTT message [" .. tostring( action.topic ) .. "] " .. tostring( payload ) )
			luup.call_action( VARIABLE.MQTT_CONNECTED[1], "Publish", { topic = action.topic, payload = payload }, action.mqttDeviceId )
		end
	}
end
...
-- TODO : use syslog ?
-- TODO : ne sauver que toutes les 30 secondes
-- TODO : limiter la taille en mémoire
local _history = {}
...
log("DEBUG params:" .. json.encode(params), "ActionGroup.execute", 5)
		...
		elseif not ActionGroup.isMatchingLevel( actionGroup, level ) then
			...
			actionGroup._context.lastUpdateTime = os.time()
		end
...
RulesInfos = {
	...
	save = function( modifiedRuleIds )
		...
		if ( file == nil ) then
			error( "File '" .. _path .. _fileName .. "' can not be written or created", "RulesInfos.save" )
			return
		end
		local rulesInfos = {}
...
Rules = {
	...
	remove = function( fileName, ruleIdx, ruleId, updateOtherRuleIdxes )
		...
		if ( ( rule ~= nil ) and ( rule.fileName == fileName ) and ( rule.idx == ruleIdx ) ) then
			...
			-- Remove conditions of the rule
			Conditions.removeRule( rule.id )
...
RulesFile = {
	...
	load = function( fileName, keepFormerRuleWithSameId, ruleIdxes, ruleNames )
		debugLogBegin( "RulesFile.load" )
		local fileName = fileName or "C_RulesEngine_Rules.xml"
		local msg = "Load rules from file '" .. fileName .. "'"
		ruleIdxes = string.split( ruleIdxes, ",", tonumber )
		if ( #ruleIdxes > 0 ) then
			msg = msg .. " and idx " .. json.encode( ruleIdxes )
		end
		ruleNames = string.split( ruleNames, "," )
		if ( #ruleNames > 0 ) then
			msg = msg .. " and ruleNames " .. json.encode( ruleNames )
		end
		log( msg, "RulesFile.load" )
	...
	--print("")
		...
		-- Add the rules parsed from XML (eventually filtered by idx or name)
		local idx = 1
			...
			for _, xmlRule in ipairs( xmltable ) do
				if ( ( type( xmlRule ) == "table" ) and ( xmlRule.tag == "block" ) and ( xmlRule.attr.type == "rule" ) ) then
					if ( ( #ruleIdxes == 0 ) or ( table.contains( ruleIdxes, idx ) ) ) then
						...
						if ( rule.id and ( rule.id > _lastRuleId ) ) then
							_lastRuleId = rule.id
						end
						rule.fileName = fileName
						rule.idx = idx -- Index in the XML file
						if ( ( #ruleNames == 0 ) or ( table.contains( ruleNames, rule.name ) ) ) then
...
-- Initialize engine
function initialize( hasToLoadRules )
	if not ( hasToLoadRules == false ) then
		-- Load rules
		History.load()
		RulesInfos.load()
		loadRulesFiles()
...
Test = {
	resetTasks = function()
		log("Reset scheduled tasks", "Test.resetTasks")
		_scheduledTasks = {}
		_nextWakeUps = {}
	end,

	resetRules = function()
		log("Reset rules", "Test.resetRules")
	...
	["ISS"] = function( params, outputFormat )
		...
		elseif string.find(path, "^/devices/[^%/]+/action/[^%/]+") then
			...
			elseif (actionName == "setAck") then
				...
				-- TODO : acknowledgement in ImperiHome can not be canceled
...
local function _initPluginInstance()
	...
	_params = {
		deviceId = g_deviceId,
		--options = Variable.getOrInit( g_deviceId, VARIABLE.PLUGIN_OPTIONS, "" ) or "",
		...
		noLoadIfDisabled = ( Variable.get( g_deviceId, VARIABLE.NO_LOAD_IF_DISABLED ) == "1" )
	}
...
local function _deferredStartup()
	...
	if _params.noLoadIfDisabled and not isEnabled() then
		UI.show( "Engine disabled : no rule load" )
	else
		initialize()
		start()
	end
...
function startup( lul_device )
	...
	if ( g_deviceId == nil ) then
		error( "deviceId is empty", "startup" )
		return
	end

	-- Update static JSON file
	if _updateStaticJSONFile( g_deviceId, _NAME .. "1" ) then
	...
	--Variable.watch( g_deviceId, VARIABLE.PLUGIN_OPTIONS, "RulesEngine.initPluginInstance" )
...
local RulesEngine = {
	...
	Tools = Tools,
	--
	getEnhancedMessage = Tools.format,
	setLabels = setLabels,
