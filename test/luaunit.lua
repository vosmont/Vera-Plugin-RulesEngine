...
-- MOD async test (vosmont)
function M.protectedCall(functionInstance, prettyFuncName)
	local ok, errMsg, stackTrace
	if (currentRunner ~= nil) then
		ok, errMsg, stackTrace = currentRunner:protectedCall( nil, functionInstance, prettyFuncName)
		if not ok then
			currentRunner:addFailure( errMsg, stackTrace )
		end
	else
		ok = false
		errMsg = "No runner active"
	end
	return ok, errMsg
end
...
local LuaUnit_MT = { __index = M.LuaUnit }
	...
	-- MOD async test (vosmont)
	function M.LuaUnit.isMethodAsyncTestName( s )
        -- return true is the name matches the name of a test method
        -- default rule is that is starts with 'Test' or with 'test'
        if string.sub(s,1,9):lower() == 'asynctest' then 
            return true
        end
        return false
    end
	...
	-- MOD async test (vosmont)
	function M.LuaUnit.isAsyncTestName( s )
        -- return true is the name matches the name of a test
        -- default rule is that is starts with 'Test' or with 'test'
        if string.sub(s,1,9):lower() == 'asynctest' then 
            return true
    ...
    function M.LuaUnit:protectedCall( classInstance , methodInstance, prettyFuncName, params)
        ...
        if classInstance then
            ...
            ok, fullErrMsg = xpcall( function () methodInstance(classInstance, unpack(params or {})) end, err_handler )
        else
            ok, fullErrMsg = xpcall( function () methodInstance(unpack(params or {})) end, err_handler )
        end
        if ok then
            return ok
        end
    ...
    function M.LuaUnit:execOneFunction(className, methodName, classInstance, methodInstance, isAsync)
		...
		self.context.testName = prettyFuncName
        self:startTest(prettyFuncName)

        -- run setUp first(if any)
        if classInstance and self.isFunction( classInstance.setUp ) then
            ok, errMsg, stackTrace = self:protectedCall( classInstance, classInstance.setUp, className..'.setUp', { prettyFuncName })
            if not ok then
        ...
        if classInstance and self.isFunction(classInstance.tearDown) then
            ok, errMsg, stackTrace = self:protectedCall( classInstance, classInstance.tearDown, className..'.tearDown', { prettyFuncName })
            if not ok then
                self:addFailure( errMsg, stackTrace )
            end
        end
    ...
    function M.LuaUnit.expandOneClass( result, className, classInstance )
        ...
        for methodName, methodInstance in sortedPairs(classInstance) do
            if M.LuaUnit.isFunction(methodInstance) and (M.LuaUnit.isMethodTestName( methodName ) or M.LuaUnit.isMethodAsyncTestName( methodName )) then
	...
	-- MOD async test (vosmont)
	function M.LuaUnit:resume()
		-- Finish the resumed test
		local classInstance = self.context.classInstance
        if classInstance and self.isFunction(classInstance.tearDown) then
            ok, errMsg, stackTrace = self:protectedCall( classInstance, classInstance.tearDown, self.context.className..'.tearDown', { self.context.testName })
            if not ok then
                self:addFailure( errMsg, stackTrace )
            end
        end
		...
		-- Execute next test
		self:runSuiteFromIndex(self.context.filteredList, (self.context.index + 1))
	end

	function M.LuaUnit:runSuiteFromIndex( filteredList, index )
		for i = (index or 1), #filteredList do
			self.context.index = i
			local v = filteredList[i]
            local name, instance = v[1], v[2]
			local isAsync = false
            if M.LuaUnit.isFunction(instance) then
				isAsync = M.LuaUnit.isAsyncTestName( name )
                self:execOneFunction( nil, name, nil, instance, isAsync )
            else 
                if type(instance) ~= 'table' then
                    error( 'Instance must be a table or a function, not a '..type(instance)..', value '..prettystr(instance))
                else
					...
					isAsync = M.LuaUnit.isMethodAsyncTestName( methodName )
					self.context.className = className
					self.context.classInstance = instance
					self.context.methodInstance = methodInstance
                    self:execOneFunction( className, methodName, instance, methodInstance, isAsync )
                end
            end
			if isAsync then
				return
    ...
    function M.LuaUnit:runSuiteByInstances( listOfNameAndInst )
		...
		self.context.filteredList = filteredList
		self:runSuiteFromIndex( filteredList, 1 )
    end
